...
MODULE_DESCRIPTION("A driver to write to a LCD display"); 
...
/**
 * @brief generate a pulse on the enable signal
 */
...
/**
 * @brief sen a command to the LCD
 * @param data: comand to send
 */
...
static int __init ModuleInit(void)
{
	...
	for (i=0; i<10; i++){
		if (gpio_direction_output(gpios[i], 0)){
			printk("lcd-driver - Error settting gpio %d to output\n", i);
			goto GpioDirectionError;
		}
	}

	/*Init the display*/
	lcd_command(0x30); /*Set the display for 8bit data interface*/
